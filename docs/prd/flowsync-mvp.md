...
> **Casos de uso:** el recorrido de una persona por cada requisito, con sus flujos alternativos y qué funciona hoy, en [casos-de-uso.md](./casos-de-uso.md). **Glosario:** las palabras del dominio con el mismo significado en PRD, spec, código y pantalla, en [glosario.md](./glosario.md).
...
*Estado, 2026-09-13 — abierto a propósito.* Se construyó como se acordó: la lista no lleva el vencimiento, y la pantalla de una tarea sí, con su señal de vencida. Decidirlo ahora sería hacerlo sin el dato que el punto pide. **Lo decidiría**: ver si el equipo abre tareas para mirar su fecha, o si se le pasan plazos que la lista habría dejado ver. Si ocurre lo segundo, la condición (b) de la fecha es la que habría que revisar.
...
*Estado, 2026-09-13 — no aplica a lo construido.* **Borrar no existe**: RF-12 (E2-10) no se ha construido y la API no registra ninguna ruta `DELETE`. Sin borrado no hay nada que recuperar de una papelera, y lo que sí se puede hacer -cambiar estado o fecha- tiene vuelta atrás (PA-7). **Se reabre al planificar E2-10**, que lo lleva anotado.
...
*Estado, 2026-09-13 — decidido el orden; la agrupación y el filtro por responsable siguen fuera.* Una corrección primero: la frase «ningún requisito fija el orden» dejó de ser cierta al construirse la lista, que ya ordenaba por recencia. Lo que faltaba era decidir si ese orden respondía la pregunta. **La vista por defecto pone primero lo que está en curso y después lo pendiente**, cada grupo de lo más reciente a lo más antiguo; acotada por un estado, recencia. En pantalla, una tarea recién creada entra la primera de las pendientes, y cambiar un estado desde la fila no mueve la fila hasta la siguiente carga. Change `lista-en-curso-primero`. **Se decidió en abstracto**, contra lo que este punto recomendaba: conviene revisarlo con una lista de uso real delante. Agrupar por persona y el filtro por responsable no cambian: siguen fuera (RF-20), ligados a PA-5.
...
*Estado, 2026-09-13 — abierto a propósito, y con más peso que antes.* Sin límite ni foco: nada impide tener varias en curso. Desde PA-3, lo que está en curso **encabeza la lista**, así que si alguien acumula cuatro, las cuatro ocupan la cabecera, y el problema que describe este punto se ve más. **Lo decidiría**: cuántas tareas en curso tiene cada persona a la vez en uso real. Si lo normal es una o dos, no hace falta nada; si se acumulan, la opción más barata es el aviso blando.
...
*Estado, 2026-09-13 — criterio escrito; la lente personal no vuelve.* Sin histórico no hay métrica posible (S-2), así que el supuesto se observa por muestreo: **una foto de la lista cada día a la misma hora durante la semana de uso**. **Se da por bueno** si la mayoría de las personas tiene algo «En curso» puesto por ella misma la mayoría de los días. **Se da por falso** si «En curso» solo cambia cuando alguien lo pide, o si las fotos muestran la lista sin tareas en curso de quien sí está trabajando. La vista «mis tareas» y el filtro por responsable **siguen fuera** (RF-20): volver a traerlos sería cambiar el producto para que el supuesto se cumpla, en lugar de comprobar si se cumple.
...
*Estado, 2026-09-13 — cerrado a medias.* **Abrir una tarea está construida** (E2-5): pantalla propia, requisito «Pantalla de una tarea» en la spec viva, y la fecha se consulta y se fija desde ahí. Deja de contar como cero. **Las otras dos siguen abiertas**, atadas a historias que no se han construido: elegir persona llega con reasignar (E2-7, RF-10) y el aviso de datos no frescos con la lista viva (E3-2, RNF-4). Cuentan cuando se planifiquen esas historias, no antes.
...
*Estado, 2026-09-13 — ratificado.* **Se puede volver de «Hecho»** a cualquiera de los otros dos estados, y **el filtro es la recuperación**: la tarea que sale de la vista por defecto avisa de adónde ha ido y se encuentra acotando por «Hecho». Marcar como hecho **sigue sin confirmación**. Lo decidió la spec viva al construirse -«Cambio de estado de cualquier tarea», escenario «Vuelta atrás desde hecho», y «Lo que sale de la vista no se pierde»- y este punto lo recoge. Al ratificarlo apareció que **ninguna prueba cambiaba una tarea desde `done`**; ahora `filtro.spec.ts` recorre el camino entero de quien se equivocó.
...
*Estado, 2026-09-13 — sigue abierto, con regla.* El caso que describe **todavía no puede darse**: borrar (E2-10), reasignar (E2-7), editar el título (E2-6) y la lista viva (E3-2) no están construidas. Lo único que hoy choca son dos cambios de estado o de fecha sobre la misma tarea, y **gana el último, sin aviso**; una pantalla de tarea abierta no se entera. **El comportamiento visible se decide antes de construir cualquiera de esas cuatro historias**, y es bloqueo de las cuatro en el backlog.
...
*Estado, 2026-09-13 — resuelto, con umbrales y lo que se midió.*

| Requisito | Umbral o protocolo | Medido el 2026-09-13 |
|---|---|---|
| **RF-6** · «con holgura», «desmedido» | **200 caracteres** se aceptan y **201** se rechazan, sin recortar | Lo fija la spec viva al construirse, con prueba |
| **RF-15** · «evidente sin cálculo mental» | La señal de vencida lleva **texto**, no solo color, y no obliga a comparar fechas | Lo fija la spec viva al construirse |
| **RF-9 / RNF-1** · «2 interacciones» | **Interacción = activar un control**: un clic, un toque, o Enter/Espacio sobre él. Mover el foco con Tab no cuenta | Cambiar el estado desde la fila cuesta **1** |
| **RNF-3** · «sin espera perceptible» | Un cambio de estado propio se ve en **menos de 100 ms**; una tarea creada, en **menos de 1 s** con el backend en la misma máquina | Estado: **4,6-8,6 ms** en cinco cambios, pintado antes de que el servidor responda (34-55 ms). Crear, contra un build de producción: **21 ms de mediana y 35 de máximo** en 20 intentos. En pantalla, con el servidor de desarrollo en modo HMR, 7 de 8 creaciones por debajo de 470 ms y **una de 1.531 ms**, que no se repitió midiendo la API: se atribuye a la recarga del servidor de desarrollo, **sin haberlo demostrado** |
| **RNF-9** · «uso razonable en tablet» | **A 768 px de ancho**: sin scroll horizontal, ningún control fuera de pantalla ni menor de 24 px | Lista, pantalla de una tarea y perfil, sin scroll horizontal y con todos los controles dentro y a tamaño de toque |
| **P-1** · «10 segundos» | El sujeto **ya ha usado la lista al menos una vez**. El cronómetro arranca con la lista cargada | Requiere un equipo real |
| **P-3** · infalsificable | Los títulos **los escribe el equipo**, no quien diseña la prueba. Si la segunda persona no detecta la tarea en curso en 10 s, **cuenta como fallo del producto**, no de las convenciones | Requiere un equipo real |
...
*Estado, 2026-09-13 — no aplica a lo construido.* **Faltan los dos factores de la multiplicación.** No hay borrado (ver PA-2), y el MVP no se publica: el repositorio no tiene ningún fichero de despliegue -ni contenedor, ni plataforma, ni workflow que publique- y el backend escucha en `localhost`. **Queda un resto que no desaparece**: el registro sigue abierto, así que quien alcance la dirección puede cambiar estados y fechas de todo el equipo. Sin borrado ese daño se deshace, pero existe. **Se reabre si ocurre cualquiera de las dos cosas**: planificar E2-10, o publicar la aplicación en un sitio alcanzable desde fuera de la máquina.
...
*Estado, 2026-09-13 — ratificado.* **En el MVP, el espacio es la instalación**: no existe como entidad, y registrarse es entrar en él. La spec viva lo fija en «Una sola lista compartida del espacio» -el mismo conjunto para cualquier cuenta- y `lista_compartida.spec.ts` lo prueba con dos cuentas que ven exactamente la misma lista. Varios espacios en una instalación sería funcionalidad nueva, y tendría su propio change.
