...
| **Navegador** | Playwright | `frontend/e2e/` | El flujo principal entero desde la pantalla de registro (`flujo.e2e.ts`), y lo que solo se ve en pantalla y ninguna otra capa veía: un token revocado, el servidor caído, dónde entra una tarea creada y que la fila no salta | Cuesta segundos por caso; por eso son pocas |
