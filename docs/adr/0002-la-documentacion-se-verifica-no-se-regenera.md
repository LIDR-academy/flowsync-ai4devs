...
`scripts/verificar-docs.mjs` corre en CI y falla la build cuando el documento y el código dejan de decir lo mismo. Hoy comprueba catorce cosas, y dos de ellas -que la regla de vencimiento tenga sus tres condiciones y que el filtro esté acotado al conjunto de estados- corresponden a defectos reales que estuvieron en el repositorio.
