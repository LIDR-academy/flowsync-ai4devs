...
> **Nota de rama (2026-08-26).** Este documento es la auditoría del repositorio tal como estaba en el **Módulo 2**, sobre `s2/start`.
> Se conserva en `s4/start` porque el registro de qué había y cómo se verificó no debería perderse al saltar de rama, pero varias de sus afirmaciones ya no describen el código de hoy.
> Para el estado actual: `README.md`, `docs/arquitectura.md` y `docs/trazabilidad.md`.
