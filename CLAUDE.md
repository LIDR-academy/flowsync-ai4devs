...
`src/App.tsx` sigue siendo la plantilla por defecto de Vite (contador + enlaces); no hay routing, gestión de estado ni cliente HTTP hacia la API todavía. El typecheck vive dentro de `npm run build`; el lint es **oxlint** (`.oxlintrc.json`), no eslint.

El formato es **Prettier** (`.prettierrc.json`: sin punto y coma, comillas simples, `printWidth` por defecto). Además hay un hook `PostToolUse` en `.claude/settings.json` que corre `prettier --write` sobre cualquier archivo bajo `frontend/` que Claude edite o cree. Depende del binario local, así que requiere haber hecho `npm install` en `frontend/`; si falta, el hook falla en silencio y no bloquea nada.
