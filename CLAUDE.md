...
Tests (Japa). Dos suites declaradas en `adonisrc.ts`: `unit` (`tests/unit/**/*.spec.ts`, timeout 2s) y `functional` (`tests/functional/**/*.spec.ts`, timeout 30s). Hoy hay **71 pruebas functional**: 26 de `auth` (registro, acceso, sesión, iniciales y mayúsculas del email), 38 de `tasks` (responsable, vencimiento, filtro, lista compartida, creación, tarea inexistente y escritura contra lectura), 5 de errores y 2 de aislamiento de la base. **`tests/unit/` no existe.** Qué escenario de la spec cubre cada una, y cuáles siguen sin cubrir, en `docs/trazabilidad.md`.
